...
def getcreate(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            instance.save()
            messages.info(request, 'Article Is Posted Successfully')
...
def getUpdate(request, pid):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            instance.article_author = u
            instance.save()
            messages.info(request, 'Article Is Updated Successfully')
            return redirect('blog:profile')
        return render(request, 'create.html', {"form": form})
    else:
        return redirect('blog:login')
    ...
    if request.user.is_authenticated:
        ...
        if author_profile:
            ...
            if form.is_valid():
                ...
                instance.name = user
                instance.save()
                messages.info(request, 'Author Profile Is Created Successfully')
                return redirect('blog:profile')
    ...
    if request.user.is_authenticated:
        if request.user.is_staff or request.user.is_superuser:
            ...
            if form.is_valid():
                instance = form.save(commit=False)
                instance.save()
                messages.info(request, 'Category is created!')
